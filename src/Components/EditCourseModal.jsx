...
function EditCourseModal({ isOpen, course, onClose, onUpdateCourse, courses }) {
  ...
  const hasConflictForRange = (day, startTime, endTime) => {
    ...
    return courses.some((existingCourse) => {
      ...
      if (existingCourse.day !== day) return false;

      const existingStart = parseHour(existingCourse.startTime);
      const existingEnd = parseHour(existingCourse.endTime);
  ...
  }, [courses, formData.day, formData.startTime, course]);
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 px-4">
      <div className="w-full max-w-2xl rounded-2xl bg-white shadow-2xl">
        <div className="flex items-center justify-between border-b px-6 py-4">
          <h2 className="text-2xl font-bold text-gray-800">Dersi Güncelle</h2>
          <button
            onClick={onClose}
        ...
        <div className="p-6">
          {error && (
            <div className="mb-4 rounded-lg bg-red-100 border border-red-300 px-4 py-2 text-red-700">
              {error}
            </div>
          )}
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div>
              ...
              <input
                ...
                className="w-full rounded-xl border px-4 py-2 outline-none focus:ring-2 focus:ring-blue-400"
              />
            </div>

            <div>
              <label className="mb-1 block font-medium text-gray-700">Gün</label>
              <select
                name="day"
                value={formData.day}
                onChange={handleChange}
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div>
                ...
                <select
                  ...
                  className="w-full rounded-xl border px-4 py-2 outline-none focus:ring-2 focus:ring-blue-400 disabled:bg-gray-100 disabled:text-gray-400"
                >
                  <option value="">Saat seçin</option>
                  {timeOptions.map((time) => {
                    const isDisabled =
            ...
            <p className="text-sm text-gray-500">
              Çakışan saat aralıkları seçim listesinde pasif görünür.
            </p>

            <div>
              <label className="mb-1 block font-medium text-gray-700">Sınıf</label>
              <input
                type="text"
                name="classroom"
            ...
            <div>
              <label className="mb-1 block font-medium text-gray-700">Renk</label>
              <select
                name="color"
            ...
            <div className="flex justify-end gap-3 pt-2">
              <button
                type="button"
                onClick={onClose}
                className="rounded-xl bg-gray-300 px-5 py-2 text-gray-800 transition hover:bg-gray-400"
              >
                İptal
              </button>
              <button
